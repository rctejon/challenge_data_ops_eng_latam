"""
Q2 TIME Implementation Runner

Este archivo ejecuta la implementación TIME-optimized de Q2 con profiling completo.
...
Arquitectura:
    - Importa la función pura desde q2_time.py
...
from q2.q2_time import q2_time


# Dataset path
DATASET_PATH = "data/raw/farmers-protest-tweets-2021-2-4.json"
PROFILE_OUTPUT = "q2_time_polars.prof"
MEMRAY_OUTPUT = "q2_time_polars_mem.bin"
...
def run_and_display_results(dataset_path: Path) -> None:
    ...
    print(f"\n{Colors.BOLD}Executing q2_time()...{Colors.RESET}")
    print("=" * 80)

    start = time.time()
    result = q2_time(str(dataset_path))
    end = time.time()
...
def main():
    ...
    print("\n" + "=" * 80)
    print(f"{Colors.BOLD}{Colors.CYAN}Q2 TIME-Optimized Implementation (Polars){Colors.RESET}")
    print("=" * 80)
