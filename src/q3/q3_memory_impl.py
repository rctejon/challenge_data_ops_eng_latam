"""
Q3 MEMORY Implementation Runner

Este archivo ejecuta la implementación MEMORY-optimized de Q3 con profiling completo.
...
Arquitectura:
    - Importa la función pura desde q3_memory.py
...
from q3.q3_memory import q3_memory


# Dataset path
DATASET_PATH = "data/raw/farmers-protest-tweets-2021-2-4.json"
PROFILE_OUTPUT = "q3_memory_polars.prof"
MEMRAY_OUTPUT = "q3_memory_polars_mem.bin"
...
def run_and_display_results(dataset_path: Path) -> None:
    ...
    print(f"\n{Colors.BOLD}Executing q3_memory()...{Colors.RESET}")
    print("=" * 80)

    start = time.time()
    result = q3_memory(str(dataset_path))
    end = time.time()

    print(f"\n{Colors.GREEN}Results - Top 10 Most Influential Users:{Colors.RESET}")
    print("-" * 80)
...
def main():
    ...
    print("\n" + "=" * 80)
    print(f"{Colors.BOLD}{Colors.CYAN}Q3 MEMORY-Optimized Implementation (Polars){Colors.RESET}")
    print("=" * 80)
