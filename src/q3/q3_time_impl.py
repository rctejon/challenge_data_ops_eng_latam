"""
Q3 TIME Implementation Runner

Este archivo ejecuta la implementación TIME-optimized de Q3 con profiling completo.
...
Arquitectura:
    - Importa la función pura desde q3_time.py
...
from q3.q3_time import q3_time


# Dataset path
DATASET_PATH = "data/raw/farmers-protest-tweets-2021-2-4.json"
PROFILE_OUTPUT = "q3_time_polars.prof"
MEMRAY_OUTPUT = "q3_time_polars_mem.bin"
...
def run_and_display_results(dataset_path: Path) -> None:
    ...
    print(f"\n{Colors.BOLD}Executing q3_time()...{Colors.RESET}")
    print("=" * 80)

    start = time.time()
    result = q3_time(str(dataset_path))
    end = time.time()
...
def main():
    ...
    print("\n" + "=" * 80)
    print(f"{Colors.BOLD}{Colors.CYAN}Q3 TIME-Optimized Implementation (Polars){Colors.RESET}")
    print("=" * 80)
