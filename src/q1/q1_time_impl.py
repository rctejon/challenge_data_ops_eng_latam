"""
Q1 TIME Implementation Runner

Este archivo ejecuta la implementación TIME-optimized de Q1 con profiling completo.
...
Arquitectura:
    - Importa la función pura desde q1_time.py
...
PROFILE_OUTPUT = "q1_time_polars.prof"
MEMRAY_OUTPUT = "q1_time_polars_mem.bin"
...
def run_and_display_results(dataset_path: Path) -> None:
    ...
    print(f"\n{Colors.BOLD}Executing q1_time()...{Colors.RESET}")
    print("=" * 80)

    start = time.time()
    result = q1_time(str(dataset_path))
    end = time.time()
...
def main():
    ...
    print("\n" + "=" * 80)
    print(f"{Colors.BOLD}{Colors.CYAN}Q1 TIME-Optimized Implementation (Polars){Colors.RESET}")
    print("=" * 80)
