"""
Q1 MEMORY Implementation Runner

Este archivo ejecuta la implementación MEMORY-optimized de Q1 con profiling completo.
...
Arquitectura:
    - Importa la función pura desde q1_memory.py
...
PROFILE_OUTPUT = "q1_memory_polars.prof"
MEMRAY_OUTPUT = "q1_memory_polars_mem.bin"
...
def run_and_display_results(dataset_path: Path) -> None:
    ...
    print(f"\n{Colors.BOLD}Executing q1_memory()...{Colors.RESET}")
    print("=" * 80)

    start = time.time()
    result = q1_memory(str(dataset_path))
    end = time.time()

    print(f"\n{Colors.GREEN}Results - Top 10 Dates with Most Tweets:{Colors.RESET}")
    print("-" * 80)
    for i, (date_obj, username) in enumerate(result, 1):
...
def main():
    ...
    print("\n" + "=" * 80)
    print(f"{Colors.BOLD}{Colors.CYAN}Q1 MEMORY-Optimized Implementation (Polars){Colors.RESET}")
    print("=" * 80)
